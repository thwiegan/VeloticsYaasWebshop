    ...
    .controller('CheckoutCtrl', ['$rootScope', '$scope', '$location', '$anchorScroll', 'CheckoutSvc','cart', 'order', '$state', '$modal', 'AuthSvc', 'AccountSvc', 'AuthDialogManager', 'GlobalData', 'ShippingSvc', 'shippingZones', 'shippingCountries', '$q', 'CartSvc', '$timeout',
        function ($rootScope, $scope, $location, $anchorScroll, CheckoutSvc, cart, order, $state, $modal, AuthSvc, AccountSvc, AuthDialogManager, GlobalData, ShippingSvc, shippingZones, shippingCountries, $q, CartSvc, $timeout) {
            ...
            var getAccount = function() {
            	...
            	$.ajax({ 
            	    ...
            	    success: function (data) { 
            	    	...
            	    	// data:
            	    	// {
            	    	// "hcpuser":{"name":"D043613","roles":["Everyone"],"locale":"en_US","groups":[],"firstname":"Manuel","lastname":"Gedack","email":"manuel.gedack@sap.com"},
            	    	// "yaasuser":{"preferredLanguage":"en_US","preferredCurrency":"USD","preferredSite":"default","metadata":{"mixins":{}},"customerNumber":"C2661175566","id":"C2661175566","addresses":[],"contactEmail":"manuel.gedack@sap.com","active":true}
            	    	// }
                        firstName = data.hcpuser.firstname;
                        lastName = data.hcpuser.lastname;
                        fullName = firstName + ' ' + lastName;

                        var title = "MS";

                        if (data.hcpuser.title == "0002") title = "MR";
                        
                        AccountSvc.account().then(function(account) {
                            $scope.order.account.email = account.contactEmail;
                            $scope.order.account.title = title;
                            $scope.order.account.firstName = firstName;
                            $scope.order.account.middleName = account.middleName;
                            $scope.order.account.lastName = lastName;
                        });
            	    }
            	});
            ...
            if (GlobalData.user.isAuthenticated) {
            	...
            	/*$.ajax({ 
            	    type: 'GET', 
            	    url: 'https://veloticswebshopa6402f209.hana.ondemand.com/api/authenticated/user', 
            	    dataType: 'json',
            	    success: function (data) { */
            	    	...
            	    	// "yaasuser":{"preferredLanguage":"en_US","preferredCurrency":"USD","preferredSite":"default","metadata":{"mixins":{}},"customerNumber":"C2661175566","id":"C2661175566","addresses":[],"contactEmail":"manuel.gedack@sap.com","active":true}
            	    	// }
            	  /*  	$scope.order.account.email = data.hcpuser.email;
                        $scope.order.account.title = '';
                        $scope.order.account.firstName = data.hcpuser.firstname;
                        $scope.order.account.middleName = '';
                        $scope.order.account.lastName = data.hcpuser.lastname;
            	    }
            	});
            	jQuery.ajaxSetup({async:true}); */ // Really bad
            }
            ...
            $scope.placeOrder = function (formValid, form) {
                ...
                if (formValid) {
                    ...
                    console.log('scope.order:'+ JSON.stringify($scope.order));
                    console.log('scope.order.cart:'+ JSON.stringify($scope.order.cart));
            ...
            function previewOrder (shipToFormValid, billToFormValid) {
            	billToFormValid = true; // dirty hack
