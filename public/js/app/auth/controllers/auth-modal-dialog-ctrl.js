 ...
 */
    .controller('AuthModalDialogCtrl', ['$rootScope', '$scope', 'AuthSvc',
        'settings', 'AuthDialogManager', 'loginOpts', 'showAsGuest', '$state', 'YGoogleSignin', '$window', 'ipCookie', 'CartSvc',
        function ($rootScope, $scope, AuthSvc, settings, AuthDialogManager, loginOpts, showAsGuest, $state, YGoogleSignin, $window, ipCookie, CartSvc) {
                    ...
                    AuthSvc.signup(authModel, loginOpts).then(
                        function (response) {
                            if (response.cookiesDisabled) {
                                ...
                                $scope.user.signup.email = hcpuserEmail;
                                $scope.user.signup.password = hcpuserPassword;
                            } else {
                                $scope.closeDialog();
                            }
                        }, function (response) {
