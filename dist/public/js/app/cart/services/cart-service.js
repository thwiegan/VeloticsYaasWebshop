        ...
        function ($rootScope, CartREST, ProductSvc, AccountSvc, $q, GlobalData) {
            ...
            function mergeAnonymousCartIntoCurrent(anonCart) {
                if (anonCart && anonCart.id) {
                    ...
                    return CartREST.Cart.one('carts', cart.id).one('merge').customPOST({ carts: [anonCart.id] }).then(function () {
                        // merge anonymous cart - will change currency if needed
                        return refreshCart(cart.id, 'merge');
                    }, function () {
                        cart.error = true;
                    });
                } else {
                    ...
                    if (cart.siteCode !== GlobalData.getSiteCode()) {
                        if (cart.id) {
                            return refreshCart(cart.id, 'site');
                        }
                    } else {
                        $rootScope.$emit('cart:updated', { cart: cart });
                        return $q.defer().resolve(cart);
                    }
                }
            }
            ...
            return {
                ...
                refreshCartAfterLogin: function (customerId) {
                    ...
                    return CartREST.Cart.one('carts', null).get({ customerId: customerId, siteCode: GlobalData.getSiteCode() }).then(function (authUserCart) {
                        // there is an existing cart - update scope instance
                        cart = authUserCart.plain();
                        return mergeAnonymousCartIntoCurrent(anonCart);
                    }, function () {
                        ...
                        if (anonCart && anonCart.id) {
                            ...
                            return CartREST.Cart.all('carts').post(cart).then(function (newCartResponse) {
