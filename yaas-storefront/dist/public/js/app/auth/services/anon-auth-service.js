    ...
    .factory('AnonAuthSvc', ['TokenSvc', '$http', '$state', '$rootScope', '$translate', 'GlobalData', 'SiteConfigSvc',
        ...
        return {
            ...
            getToken: function(){
                ...
                if(!inProgress) {
                    inProgress = true;
                    // Set Hybris Tenant to an empty string. Don't know why but it works ;-)
                    //$http.get(siteConfig.apis.account.baseUrl + '/auth/anonymous/login?client_id=' + GlobalData.store.clientId + '&redirect_uri=' + encodeURIComponent(GlobalData.store.redirectURI) + '&hybris-tenant=').then( function(response){
